...
async def classify_bot_intent(user_query: str, context: Optional[Dict[str, Any]] = None, is_admin: bool = False) -> Dict[str, Any]:
    """
    ...
    # Base intents available to everyone (WhatsApp)
    allowed_intents = """
    ...
    # Admin-only high-end features (Webapp only)
    admin_intents = """
    ...
    if is_admin:
        allowed_intents += admin_intents

    system_prompt = f"""You are an intent classifier for a Finance Intelligence Bot.
    ...
    # Use explicit replacement to avoid KeyError with JSON braces {} in prompt
